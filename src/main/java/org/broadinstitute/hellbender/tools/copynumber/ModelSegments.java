...
public final class ModelSegments extends CommandLineProgram {
    ...
    private void logHeapUsage(final String phase) {
        ...
        logger.debug("Used memory (MB) after " + phase + ": " + (runtime.totalMemory() - runtime.freeMemory()) / mb);
    }
    ...
    /**
     * Performs reading and validation of data.  Regardless of {@code dataMode}, the respective lists for the
     * copy-ratio and allele-fraction data for the case samples will be of the same length and populated;
     * empty collections will be used to impute missing data using the same sample order as the non-missing data.
     * This is done so that a {@link MultisampleMultidimensionalKernelSegmenter} can be used in all scenarios.
     *
    ...
    private final class ModelSegmentsData {
        ...
        final SimpleIntervalCollection segments;
        final List<SampleLocatableMetadata> metadataPerSample;
        ...
        private ModelSegmentsData() {
            ...
            switch (dataMode) {
                case COPY_RATIO_ONLY:
                    denoisedCopyRatiosPerSample = inputDenoisedCopyRatiosFiles.stream()
                            .map(CopyRatioCollection::new)
                            .collect(Collectors.toList());
                    metadataPerSample = denoisedCopyRatiosPerSample.stream()
                            .map(CopyRatioCollection::getMetadata)
                            .collect(Collectors.toList());
                    allelicCountsPerSample = metadataPerSample.stream()
                            .map(m -> new AllelicCountCollection(m, Collections.emptyList()))
                            .collect(Collectors.toList());
                    break;
                case ALLELE_FRACTION_ONLY:
                    allelicCountsPerSample = inputAllelicCountsFiles.stream()
                            .map(AllelicCountCollection::new)
                            .collect(Collectors.toList());
                    metadataPerSample = allelicCountsPerSample.stream()
                            .map(AllelicCountCollection::getMetadata)
                            .collect(Collectors.toList());
                    denoisedCopyRatiosPerSample = metadataPerSample.stream()
                ...
                case COPY_RATIO_AND_ALLELE_FRACTION:
                    ...
                    allelicCountsPerSample = inputAllelicCountsFiles.stream()
                            ...
                            .collect(Collectors.toList());
                    metadataPerSample = IntStream.range(0, inputDenoisedCopyRatiosFiles.size()).boxed()
                            .map(i -> CopyNumberArgumentValidationUtils.getValidatedMetadata(   //check that sample metadata matches across copy-ratio and allele-fraction data
    ...
    private void setModesAndValidateArguments() {
        ...
        Utils.validateArg(!(inputDenoisedCopyRatiosFiles == null && inputAllelicCountsFiles == null),
                "Must provide at least one denoised-copy-ratios file or allelic-counts file.");
        Utils.validateArg(!(inputAllelicCountsFiles == null && inputNormalAllelicCountsFile != null),
        ...
        if (inputAllelicCountsFiles == null) {
            dataMode = DataMode.COPY_RATIO_ONLY;
        } else if (inputDenoisedCopyRatiosFiles == null) {
